...
class SortBySalary implements Comparator<Customer>{
	@Override
	public int compare(Customer firstObject, Customer secondObject){
		return ((Double)firstObject.getSalary()).compareTo((Double)secondObject.getSalary());
	}
}
...
class Customer implements Comparable<Customer>{
	...
	public int compareTo(Customer object){
		// both work for Ascii Order
		//return this.name.compareTo(object.name);  // Asc
		//return object.name.compareTo(this.name); // Desc
		// Dictionary Order
...
public class CRUDDemo {
	...
	public static void main(String[] args) {
		...
		if(choice ==8){
			// Lambda 
			Collections.sort(CustomerOperations.customerList
					,(firstObject,secondObject)->
			firstObject.getName().
			compareToIgnoreCase(secondObject.getName()));
			
			// Anonymous 
			Collections.sort(CustomerOperations.customerList, new Comparator<Customer>() {
				@Override
				public int compare(Customer firstObject, Customer secondObject){
			...
			//Collections.sort(CustomerOperations.customerList); // Always Call Comparable
			//Collections.sort(CustomerOperations.customerList, 
					//new SortBySalary()); // Always Call Comparator
			//Collections.sort(CustomerOperations.customerList,
					//new SortByName());
